...
<head>
    ...
    <title>{{ config('app.name') }} | Question - Create</title>
...
<section class="container mb-5">
    <h1 class="text-success text-center m-5">Quiz - Create</h1>

    <form action="{{ route('admin.question.store') }}" method="POST">
        @csrf
        <div class="mb-3">
            <label for="content" class="form-label text-success">Content</label>
            <input type="text" class="form-control" id="content" name="content" value="{{ old('content') }}" autofocus>
        </div>
        <div class="mb-3">
            <label for="grade" class="form-label text-success">Grade</label>
            <input type="number" class="form-control" id="grade" name="grade" value="{{ old('grade') }}">
        </div>
        <div class="mb-3">
            <label for="correct_answers" class="form-label text-success">Correct Answers</label>
            <input type="number" class="form-control" id="correct_answers" name="correct_answers" value="{{ old('correct_answers') }}">
        </div>
